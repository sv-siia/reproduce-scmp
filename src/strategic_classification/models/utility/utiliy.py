...
from strategic_classification.models.utility.utilityccp import UtilityCCP
from strategic_classification.models.utility.utilitydelta import UtilityDELTA
...
class MyUtility(torch.nn.Module):
    def __init__(self, x_dim, train_slope, eval_slope, strategic=False, lamb=0):
        ...
        self.lamb = lamb
        self.ccp = UtilityCCP(x_dim)
        self.delta = UtilityDELTA(x_dim)
    ...
    def optimize_X(self, X):
        return self.ccp.optimize_X(X, self.w, self.b, self.eval_slope)
    ...
    def calc_utility(self, X, X_opt, evaluation=False):
        return torch.mean(self.calc_utility_aux(X, X_opt, evaluation))
    
    def calc_utility_aux(self, X, X_opt, evaluation=False):
        """
    ...
    def fit(self, X, Y, Xval, Yval, Xtest, Ytest, opt, opt_kwargs={"lr":1e-3}, batch_size=128, epochs=100, verbose=False, callback=None, calc_train_errors=False, comment=None):
        ...
        now = datetime.now()
        path = "./models/utility/" + now.strftime("%d-%m-%Y_%H-%M-%S")
...
def train_utility(dataset_path: str, epochs: int = 10, batch_size: int = 64, model_checkpoint_path: str = "models/utility"):
    """
    Train a utility model with the given parameters.
    Args:
    ...
    for lamb in lambda_range:
        ...
        strategic_model = MyUtility(x_dim, TRAIN_SLOPE, EVAL_SLOPE, strategic=True, lamb=lamb)
